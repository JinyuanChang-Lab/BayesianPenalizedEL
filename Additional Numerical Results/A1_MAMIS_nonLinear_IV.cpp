...
double dmt_arma(arma::vec x, arma::vec mean, arma::mat Sigma, double df){
  ...
  arma::mat pinv_Sigma = arma::pinv(Sigma);
  arma::mat temp = 1 + (x - mean).t() * pinv_Sigma * (x - mean) / df; 
...
// [[Rcpp::export]]
Rcpp::List MAMIS_1_arma(arma::vec mu_init, arma::vec y, arma::mat x, arma::mat z, double nu){
  int p = x.n_cols;
  double r = z.n_cols;
  double n = z.n_rows;
  double thresh = 1.0/n;
  ...
  for(int k=0; k<K_iter; k++){
    for(int i=0; i<Nk(k); i++){
      ...
      if (max(abs(beta))<=10){
        pos = exp(-1.0 * U_beta_arma(lambda, Gee, nu, n, thresh) - U_pri_arma(beta, mean_pri, 0.5)); 
      }
      else {
        pos = 0.0;
      }
...
// [[Rcpp::export]]
Rcpp::List MAMIS_2_arma(arma::vec mu_init, arma::vec y, arma::mat x, arma::mat z, double nu){
  int p = x.n_cols;
  double r = z.n_cols;
  double n = z.n_rows;
  double thresh = 1.0/n;
...
// [[Rcpp::export]]
Rcpp::List MAMIS_3_arma(arma::vec mu_init, arma::vec y, arma::mat x, arma::mat z, double nu){
  int p = x.n_cols;
  double r = z.n_cols;
  double n = z.n_rows;
  double thresh = 1.0/n;
