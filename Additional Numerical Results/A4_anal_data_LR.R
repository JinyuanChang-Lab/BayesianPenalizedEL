...
library(ggplot2)
######################### r = 50 #########################
res <- read.csv(paste0('result/LR_all_nu_0.03_r_50.csv')
                , header = F)
...
######################### r = 70 #########################
res <- read.csv(paste0('result/LR_all_nu_0.03_r_70.csv')
                , header = F)
...
Data <- NULL
  ...
  for (j in seq(20,120,10)){
    res <- read.csv(paste0('result/LR_dis_', j,'_r_', r, '.csv'), header = F)
    res <- res[-1,]
    res <- res[, -1]
    res <- as.matrix(res)
...
p + geom_line(aes(color = label))+geom_point(aes(shape = label, color = label), size=2)+ ylim(0.03, 0.32)+ scale_shape_manual(values = c(15,16)) + theme_bw() + 
