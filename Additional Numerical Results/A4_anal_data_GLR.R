...
library(ggplot2)
######################### r = 50 #########################
res <- read.csv(paste0('result/GLR_all_nu_0.03_r_50.csv')
                , header = F)
...
write.csv(dis, paste0('result/dis_120_r_50.csv'))



######################### r = 70 #########################
res <- read.csv(paste0('result/GLR_all_nu_0.03_r_70.csv')
                , header = F)
...
write.csv(dis, paste0('result/dis_120_r_70.csv'))
...
Data <- NULL
  ...
  for (j in seq(20,120,10)){
    res <- read.csv(paste0('result/GLR_dis_', j,'_r_', r, '.csv'), header = F)
    res <- res[-1,]
    res <- res[, -1]
    res <- as.matrix(res)
...
p + geom_line(aes(color = label))+geom_point(aes(shape = label, color = label), size=2)+ ylim(0.01, 0.32)+ scale_shape_manual(values = c(15,16)) + theme_bw() + 
  theme(legend.position = c(0.7,0.85), panel.grid = element_blank(), legend.text = element_text(size = 20, family = "serif"), legend.title = element_blank()) + 
