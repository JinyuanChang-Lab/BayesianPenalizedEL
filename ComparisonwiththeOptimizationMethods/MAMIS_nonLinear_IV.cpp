...
arma::mat IVfun_arma(arma::vec beta, arma::vec y, arma::mat x, arma::mat z){
  arma::vec residual = y - sin(x * beta);
...
Rcpp::List MAMIS_arma(arma::vec mu_init, arma::vec y, arma::mat x, arma::mat z, double nu){
  ...
  double S_K = sum(Nk), df = 3.0, pos = 0.0, phi = 0.0, deno_Nk, S_K_1 = sum(Nk.head(10)), S_K_2 = sum(Nk.head(14));
  ...
  arma::vec mu(p, arma::fill::zeros), weight(S_K, arma::fill::zeros), POS(S_K), lambda(r), beta(p), beta_1(p), beta_2(p), beta_3(p),
            weight_1(S_K_1, arma::fill::zeros), weight_2(S_K_2, arma::fill::zeros);
  ...
  weight_1 = POS.head(S_K_1) / (sum(DENO.submat(0, 0, (S_K_1-1), 9),1) / S_K_1);
  if(sum(weight_1)!=0){
    beta_1 = beta_iter.submat(0, 0, 1, (S_K_1 - 1)) * weight_1 / sum(weight_1); // MAMIS estimator
  }
  else{
    beta_1 = mu;
  }
  
  weight_2 = POS.head(S_K_2) / (sum(DENO.submat(0, 0, (S_K_2-1), 13),1) / S_K_2);
  if(sum(weight_2)!=0){
    beta_2 = beta_iter.submat(0, 0, 1, (S_K_2 - 1)) * weight_2 / sum(weight_2); // MAMIS estimator
  }
  else{
    beta_2 = mu;
  }
  ...
  return List::create(Named("beta_3") = beta_3, Named("beta_2") = beta_2, Named("beta_1") = beta_1, 
                      Named("weight") = weight, Named("weight_2") = weight_2, Named("weight_1") = weight_1, 
