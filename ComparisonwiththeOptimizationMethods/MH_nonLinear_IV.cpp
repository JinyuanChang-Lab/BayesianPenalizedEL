...
// [[Rcpp::export]]
Rcpp::List MH_arma(arma::vec beta, arma::vec y, arma::mat x, arma::mat z, double nu){
  int p = beta.n_elem;
  double r = z.n_cols;
  double n = z.n_rows;
  double thresh = 1.0/n;
  ...
  int iter;
  bool verbose=false;
  double sig = (8+(n-120)/80)/sqrt(n*log(r)), U_value, curr_U = 0.0, pro_U = 0.0, ratio, aplha, u, accept = 0.0, accept_rio = 0.0;
  ...
  beta_mean_1 = arma::mean(beta_iter.cols(501, 1500), 1);           // Mean_1: 501！！1500;   
  
  beta_mean_2 = arma::mean(beta_iter.cols(501, 2500), 1);         // Mean_2: 501！！2500; 
  
  beta_mean_3 = arma::mean(beta_iter.tail_cols(3000), 1);         // Mean_3: 501！！3500; 
  
  accept_rio = accept / iter_sl;
  
  return List::create(Named("beta_mean_1") = beta_mean_1, Named("beta_mean_2") = beta_mean_2, Named("beta_mean_3") = beta_mean_3,
                      Named("accept_rio") = accept_rio, Named("beta_iter") = beta_iter);
}
