...
// [[Rcpp::export]]
Rcpp::List MH_arma(arma::vec beta, arma::mat y, arma::vec v, arma::mat a, arma::mat h, arma::rowvec X_bar, arma::rowvec N_nF, double nu){
  int p = beta.n_elem;
  double r = y.n_cols;
  double n = y.n_rows;
  double thresh = 1.0/n;
  ...
  int iter;
  bool verbose=false;
  double sig = 1/sqrt(n*log(r)), U_value, curr_U = 0.0, pro_U = 0.0, ratio, aplha, u, accept = 0.0, accept_rio = 0.0;
  arma::mat curr_Gee(n, r), pro_Gee(n, r), beta_iter(p, (iter_sl + 1), arma::fill::zeros);
  arma::vec lambda(r), curr_lambda(r), pro_lambda(r), curr_beta(p), pro_beta(p), beta_mean_1(p), beta_mean_2(p), beta_mean_3(p); 
  
  beta_iter.col(0) = beta;
  curr_beta = beta;
  for (iter=0; iter<iter_sl; iter++){
    curr_Gee = GEE_arma(curr_beta, y, v, a, h, X_bar, N_nF);
    ...
    if (pro_beta(0)<1.5 || pro_beta(0)>10 || pro_beta(1)<0.5 || pro_beta(1)>5 || pro_beta(2)<0.1 || pro_beta(2)>5 || 
        pro_beta(3)<0.1 || pro_beta(3)>5 || pro_beta(4)<-0.9 || pro_beta(4)>0.9){
      ratio = 0.0;
    }
    else {
      pro_Gee = GEE_arma(pro_beta, y, v, a, h, X_bar, N_nF);
      if(pro_Gee.has_nan()) {
        ratio = 0.0;
      }
      else {
        ...
        if(std::isinf(pro_U) || pro_lambda.has_nan()){ 
          ratio = 0.0;
        }
        else {
    ...
    Rcout<<curr_beta<<endl;

  }
  
  beta_mean_3 = arma::mean(beta_iter.tail_cols(3000), 1);         // Mean_3: 501¡ª¡ª3500; 
