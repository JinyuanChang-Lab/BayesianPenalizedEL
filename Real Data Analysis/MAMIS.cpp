...
Rcpp::List EKK_simulator(arma::vec b, double S, double m, arma::vec v, arma::mat a, arma::mat h, arma::rowvec X_bar, arma::rowvec N_nF){ 
  
  double k1, k2, theT = b(0), lamb = b(1), sigA = b(2), sigE = b(3), rho  = b(4);
  arma::rowvec sig_E_nF;
  arma::vec u_bar_F, u_bar, u;
  arma::mat alpha, eta, ratio, component0, u_n_bar, u_bar_X, delta_nF_tem, component1, component2, X_nF0, X_nF;
  
  // step 1
  k1 = (theT / (theT - 1) - theT / (theT + lamb - 1)) * exp( 0.5 * (sigA + 2*rho*sigA*sigE*(theT -1) + sigE * pow((theT - 1), 2)));
  k2 = exp( 0.5 * pow((theT * sigE), 2));
  
  // step 2
  sig_E_nF = (k2/k1) * X_bar;
  
  // step 3
  alpha = exp( sigA * sqrt( 1 - pow(rho,2)) * a + sigA * rho *  h);
  eta   = exp( sigE * h);
  ratio = alpha / eta;
  
  // step 4
  component0 = arma::repmat( N_nF, S, 1);
  component0 = component0 % eta;
  u_n_bar = pow(component0, theT);
  
  // step 5
  u_bar_X = max(u_n_bar.tail_cols(126), 1);
  u_bar_F = u_n_bar.col(0);
  u_bar   =  arma::min ( u_bar_X, u_bar_F);
  ...
  // step 8
  delta_nF_tem = arma::repmat(u, 1, m);
  arma::umat delta_nF = (delta_nF_tem <= u_n_bar); 
  
  component1 = arma::repmat(u, 1, m);
  component1 = component1 / u_n_bar;
  component2 = arma::repmat(sig_E_nF, S, 1);
  
  X_nF0 = ratio % pow((1-component1), (lamb/theT)) % pow(component1, (-1.0/theT)) % component2;
  X_nF = delta_nF % X_nF0;
  
  X_nF.replace(arma::datum::nan, 0);
  
  return List::create(Named("delta_nF") = delta_nF, Named("X_nF") = X_nF);
}



arma::mat EKK_reduced2(arma::vec b, arma::vec v, arma::mat a, arma::mat h, arma::rowvec X_bar, arma::rowvec N_nF){
  
  double Sn = a.n_rows;
  double m = a.n_cols;
  arma::rowvec N_nF_art, X_bar_pred;
  Rcpp::List EKK;
  
  EKK = EKK_simulator(b, Sn, m, v, a, h, X_bar, N_nF);
  
  arma::umat delta_nF = EKK["delta_nF"];
  arma::mat delta_nF_int(Sn, m, arma::fill::ones);
  delta_nF_int = delta_nF_int % delta_nF;
  
  arma::mat X_nF = EKK["X_nF"];
  
  N_nF_art = sum(delta_nF_int, 0);
  
  X_bar_pred = sum(X_nF, 0) / N_nF_art;
  X_bar_pred.replace(arma::datum::nan, 0);
  X_bar_pred = X_bar_pred.tail(m-1);
...
// [[Rcpp::export]]
arma::mat GEE_arma(arma::vec b, arma::mat y, arma::vec v, arma::mat a, arma::mat h, arma::rowvec X_bar, arma::rowvec N_nF){
  double S = y.n_rows;
  arma::mat  yArt_sell = EKK_reduced2(b, v, a, h, X_bar, N_nF);
  arma::mat Gee = y - arma::repmat(yArt_sell, S, 1);
...
double dmt_arma(arma::vec x, arma::vec mean, arma::mat Sigma, double df){
  ...
  arma::mat temp = 1 + (x - mean).t() * arma::pinv(Sigma) * (x - mean) / df; 
...
// [[Rcpp::export]]
Rcpp::List MAMIS_arma(arma::vec mu_init, arma::mat y, arma::vec v, arma::mat a, arma::mat h, arma::rowvec X_bar, arma::rowvec N_nF, double nu){
  int p = mu_init.n_elem;
  double r = y.n_cols;
  double n = y.n_rows;
  double thresh = 0.003;
  ...
  double S_K = sum(Nk), df = 3.0, pos = 0.0, phi = 0.0, deno_Nk, temp;
  ...
  arma::vec mu(p, arma::fill::zeros), weight(S_K, arma::fill::zeros), POS(S_K), lambda(r), beta(p), beta_3(p);
  
  mu = mu_init;
  
  MU.col(0) = mu;
  SIG.submat(0, 0, 4, 4) = Sigma;
  ...
  for(int k=0; k<K_iter; k++){
    for(int i=0; i<Nk(k); i++){
      ...
      Gee = GEE_arma(beta, y, v, a, h, X_bar, N_nF);
      if (Gee.has_nan() || beta(0)<1.5 || beta(0)>10 || beta(1)<0.5 || beta(1)>5 || beta(2)<0.1 || beta(2)>5 || 
          beta(3)<0.1 || beta(3)>5 || beta(4)<-0.9 || beta(4)>0.9){
        pos = 0.0;
      }
      else {
        ...
        if(std::isinf(pos) || lambda.has_nan()){
          pos = 0.0;
        }
      }
      
      if (pos == arma::datum::nan){
        pos = 0.0;
      }
      ...
      temp = pos / phi;
      if(std::isinf(temp)){
        weight(i+sum(Nk.head(k))) = 0.0;
      }
      else {
        weight(i+sum(Nk.head(k))) = temp;
      }
    ...
    if(deno_Nk!=0){
      ...
      Sigma = (temp * temp.t()) / deno_Nk + Epi*0.1;
      SIG.submat(0, (p*(k+1)), 4, (p*(k+1)+4)) = Sigma;
    }
    Rcout<<Sigma<<endl;
  }
  ...
  for(int j=0; j<S_K; j++){
    ...
    for(int k=0; k<K_iter; k++){
      mu = MU.col(k);
      Sigma = SIG.submat(0, (p*k), 4, (p*k+4));
  ...
  return List::create(Named("beta_3") = beta, 
                      Named("weight") = weight, 
                      Named("beta_iter") = beta_iter, Named("MU") = MU, Named("SIG") = SIG);
}
