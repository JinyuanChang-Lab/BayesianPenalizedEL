...
Rcpp::List MH_arma(arma::vec beta, arma::vec y, arma::mat x, arma::mat z, double nu){
  ...
  int iter;
  bool verbose=false;
  double sig = 0.01, U_value, curr_U = 0.0, pro_U = 0.0, ratio, aplha, u, accept = 0.0, iter_num = 0.0;
  arma::mat curr_Gee(n, r), pro_Gee(n, r), beta_iter(p, (iter_sl + 1), arma::fill::zeros);
  arma::vec lambda(r), curr_lambda(r), pro_lambda(r), curr_beta(p), pro_beta(p), //beta_mean_1(p), beta_mean_2(p), 
            beta_mean_3(p); 
  ...
  return List::create(Named("iter_num") = iter_num, Named("accept") = accept);
}
