...
arma::mat NS_fun_arma(double beta, arma::vec x, double r){
  double n = x.n_rows;
  double temp, var;
  arma::mat Gee(n, r); 
  
  for(int j=0; j<r; j++){
    temp = double_factorial_arma(2*j+1) * pow(beta, (j+1)); 
    var = double_factorial_arma(4*j+3) * pow(beta, (2*j+2)) - pow(temp, 2);
    Gee.col(j) = (pow(x, (2*j+2)) - temp) / sqrt(var);
  }
...
arma::vec normal_den_arma(arma::vec x, double beta){
  arma::vec obj = exp(- x%x / (2*beta)) / sqrt(2*arma::datum::pi*beta);
  return obj; 
}

// [[Rcpp::export]]
double Pos_beta_arma(double beta, arma::vec x){
  double obj = arma::prod(normal_den_arma(x, beta)) * exp(100);
  return obj; 
}
...
double U_PEL_arma(arma::vec lambda, arma::mat Gee, double nu, double n, double thresh){
  ...
  double obj = n * log(n) + arma::sum(logstar_arma(lam_g, n, thresh)) - n * nu * arma::norm(lambda,1) - n*5;
  return obj; 
}
...
double U_EL_arma(arma::vec lambda, arma::mat Gee, double n, double thresh){
  arma::vec lam_g = Gee * lambda + 1.0;
  double obj = n * log(n) + arma::sum(logstar_arma(lam_g, n, thresh)) - n*5;
  return obj; 
}
...
Rcpp::List MH_PEL_arma(double beta, arma::vec x, double r, double nu){
  ...
  for (iter=0; iter<iter_sl; iter++){
    curr_Gee = NS_fun_arma(curr_beta, x, r);
    ...
    if (pro_beta>0 && pro_beta<=4){
      pro_Gee = NS_fun_arma(pro_beta, x, r);
...
Rcpp::List MH_Pos_arma(double beta, arma::vec x){
  ...
  for (iter=0; iter<iter_sl; iter++){
    pro_beta = curr_beta + sig * arma::randn<double>();
    if (pro_beta>0 && pro_beta<=4){
      ratio = Pos_beta_arma(pro_beta, x) / Pos_beta_arma(curr_beta, x);
    }
...
// [[Rcpp::export]]
double PEL_beta_arma(double beta, arma::vec x, double r, double nu){
  arma::mat Gee = NS_fun_arma(beta, x, r);
...
// [[Rcpp::export]]
double EL_beta_arma(double beta, arma::vec x, double r){
  arma::mat Gee = NS_fun_arma(beta, x, r);
  double nu = 0.0;
  arma::vec lambda = optimal_lam(Gee, nu, 1e-9, 1e-3, 5000, false, "identity");
  double n = x.n_rows;
